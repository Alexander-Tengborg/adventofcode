...
def extrapolateNextValue(history):
    ...
    while not all(num == 0 for num in histories[-1]): # any(histories[-1]) WOULD ALSO WORK
    ...
    for i in reversed(range(len(histories) - 1)):
        ...
        # extrapolated value = first value of the current history - the first value of the next history
        extrapolated_value = histories[i][0] - histories[i+1][0]
        histories[i].insert(0, extrapolated_value)
