...
def get_reachable_garden_plots(map, starting_position, max_steps):
    ...
    while queue:
        ...
        for new_y, new_x, new_steps in neighbours:
            tile_y = abs(new_y) % len(map)
            tile_x = abs(new_x) % len(map[0])
            if map[tile_y][tile_x] == '#' or new_steps > max_steps:
                continue
...
visited = get_reachable_garden_plots(map, starting_position, 500)
